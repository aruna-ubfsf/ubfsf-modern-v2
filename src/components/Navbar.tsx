    ...
    <nav className="bg-[#1a1a1a] text-white py-3 px-6 sticky top-0 z-[100] shadow-2xl border-b border-white/5">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <span className="font-black text-xl uppercase group-hover:text-[#FFB81C] transition-colors">UBFSF</span>
        </Link>

        {/* DESKTOP NAV */}
        <div className="hidden md:flex items-center gap-8 text-[10px] font-black uppercase tracking-widest">
          <Link href="/" className="hover:text-[#FFB81C]">Home</Link>
          
          {/* ABOUT DROPDOWN */}
          <div className="relative" onMouseEnter={() => setIsAboutOpen(true)} onMouseLeave={() => setIsAboutOpen(false)}>
            <button className="flex items-center gap-1 hover:text-[#FFB81C] uppercase">About <ChevronDown size={10} /></button>
            {isAboutOpen && (
              <div className="absolute top-full left-0 w-56 bg-[#1a1a1a] border border-white/10 py-3 rounded-b-lg shadow-xl">
                {aboutLinks.map(link => (
                  <Link 
                    key={link.title} 
                    href={link.href} 
                    className="block px-6 py-2.5 hover:bg-white/5 hover:text-[#FFB81C] transition-colors text-[11px]"
                  >
                    {link.title}
                  </Link>
                ))}
          ...
          <div className="relative" onMouseEnter={() => setIsProgramsOpen(true)} onMouseLeave={() => setIsProgramsOpen(false)}>
            <button className="flex items-center gap-1 hover:text-[#FFB81C] uppercase">Programs <ChevronDown size={10} /></button>
            {isProgramsOpen && (
          ...
          <div className="relative" onMouseEnter={() => setIsBlogOpen(true)} onMouseLeave={() => setIsBlogOpen(false)}>
            <button className="flex items-center gap-1 hover:text-[#FFB81C] uppercase">Blog <ChevronDown size={10} /></button>
            {isBlogOpen && (
              <div className="absolute top-full left-0 w-48 bg-[#1a1a1a] border border-white/10 py-3 rounded-b-lg shadow-xl">
                <Link href="/blog" className="block px-6 py-2.5 hover:bg-white/5 hover:text-[#FFB81C] transition-colors text-[11px]">Blog Home</Link>
                <a href="https://the-new-wave-newsletter-ubfsf.beehiiv.com/" target="_blank" rel="noopener noreferrer" className="block px-6 py-2.5 hover:bg-white/5 hover:text-[#FFB81C] transition-colors text-[11px]">Newsletter</a>
              </div>
            )}
          </div>

          <Link href="/contact" className="hover:text-[#FFB81C]">Contact</Link>
        </div>
        ...
        <Link href="/donate" className="hidden md:block">
          <button className="bg-[#FFB81C] text-black text-[10px] font-black px-5 py-2.5 hover:bg-yellow-500 transition-all">DONATE NOW</button>
        </Link>
      </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#1a1a1a] py-6 px-6 space-y-4 text-sm font-bold uppercase border-t border-white/10 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-2 pl-4">
              {aboutLinks.map(link => (
                <Link 
                  key={link.title} 
                  href={link.href} 
                  className="block text-xs hover:text-[#FFB81C] transition-colors"
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2 pl-4">
              <Link href="/blog" className="block text-xs hover:text-[#FFB81C]" onClick={() => setIsMobileMenuOpen(false)}>Blog Home</Link>
              <a href="https://the-new-wave-newsletter-ubfsf.beehiiv.com/" target="_blank" rel="noopener noreferrer" className="block text-xs hover:text-[#FFB81C]">Newsletter</a>
            </div>
          </div>

          <Link href="/contact" className="block" onClick={() => setIsMobileMenuOpen(false)}>Contact</Link>
